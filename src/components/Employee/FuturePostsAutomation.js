...
export default function FuturePostsAutomation() {
  ...
  const [keywords, setKeywords] = useState(['Transform', 'Diet', 'Fitness']);
 ...
 const loadAutomationForEdit = async () => {
    try {
      setIsLoadingAutomation(true);
        const response = await axios.get(`${baseUrl}/future/automation/config`, {
                  withCredentials: true,
                });
     ...
     if (response.data?.exists) {
        ...
        const originalState = {
          ...
          isReplyAvailable: data.hasReply || false,
          replies: [...(data.replyComments || [
            "Thanks for the comment, Please check DM 🙂",
        ...
        setIsReplyAvailable(data.hasReply);
...
useEffect(() => {
 ...
 const handleToggleStatus = async () => {
  try {
    ...
    await axios.patch(
      `${baseUrl}/future/automation/status`,
      { isActive: newStatus },
      { withCredentials: true }
    );
...
const handleLaunchAutomation = async () => {
    ...
    try {
      ...
      await api.post("/future/automation/config", payload);
